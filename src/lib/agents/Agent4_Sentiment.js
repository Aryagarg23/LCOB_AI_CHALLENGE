...
export async function agent4_Sentiment(brandName, recentReviews, businessAgeMonths = undefined, userAnswer = null) {
  ...
  try {
    ...
    const { object } = await generateObject({
      ...
      schema: z.object({
        ...
        clarificationQuestion: z.string().nullable().describe(isPreLaunch ? 'Since this is a PRE-LAUNCH business, do NOT ask about past customer feedback. Ask an open-ended question about how they want their future customers to feel. Otherwise null.' : 'If brand is unfindable online and no reviews provided, ask about customer feedback. Otherwise null.'),
      }),
...
1. "${brandName} reviews" or "${brandName} yelp" for customer feedback
2. "${brandName} social media" or "${brandName}" for general perception

If the business is new/unknown online, search for sentiment about similar businesses in that area.
Cite every URL. Quote specific review snippets in reasoning.

User-Provided Reviews: "${recentReviews || 'None provided'}"
${isPreLaunch ? 'Context: THIS IS A PRE-LAUNCH / NEW BUSINESS with no existing reviews.' : ''}
